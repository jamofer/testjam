"""Jinja-rendered email templates: rendering, escape, footer wiring, locales."""
...
def test_assigned_subject_contains_app_name_and_title():
    subject, _, _, _ = email_templates.render(ASSIGNED, _base_context())
    assert subject == "[Testjam] You were assigned to 'Smoke run'"


def test_assigned_html_includes_actor_and_link():
    _, html, _, _ = email_templates.render(ASSIGNED, _base_context())
...
def test_assigned_text_includes_actor_and_link():
    _, _, text, _ = email_templates.render(ASSIGNED, _base_context())
    assert "alice assigned you to 'Smoke run'." in text
    assert "https://qa.example.com/executions/1/run" in text


def test_assigned_message_is_short_and_named():
    _, _, _, message = email_templates.render(ASSIGNED, _base_context())
    assert message == "alice assigned you to 'Smoke run'"


def test_html_autoescapes_malicious_title():
    context = _base_context()
    context["subject_object"] = "<script>alert('xss')</script>"
    _, html, _, _ = email_templates.render(FINISHED, context)
...
def test_html_autoescapes_malicious_actor():
    ...
    _, html, _, _ = email_templates.render(ASSIGNED, context)
...
def test_text_does_not_escape():
    ...
    _, _, text, _ = email_templates.render(FINISHED, context)
...
def test_finished_renders_summary_block_when_provided():
    ...
    _, html, text, _ = email_templates.render(FINISHED, context)
...
def test_finished_renders_without_summary():
    _, html, _, _ = email_templates.render(FINISHED, _base_context())
...
def test_failed_uses_failure_styling_and_summary():
    ...
    subject, html, text, _ = email_templates.render(FAILED, context)
...
def test_footer_reason_and_preferences_link_present():
    _, html, text, _ = email_templates.render(ASSIGNED, _base_context())
...
def test_footer_link_falls_back_when_no_site_url():
    ...
    _, html, text, _ = email_templates.render(ASSIGNED, context)
...
def test_spanish_locale_uses_es_subject_and_message():
    subject, html, text, message = email_templates.render(
        ASSIGNED, _base_context(), locale="es",
    )
    assert subject == "[Testjam] Te han asignado a «Smoke run»"
    assert message == "alice te ha asignado a «Smoke run»"
    assert "te ha asignado" in text
    assert "Abrir ejecución" in html
