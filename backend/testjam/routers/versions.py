...
def list_versions(id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def create_version(id: int, body: ProjectVersionCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
