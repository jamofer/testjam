...
def list_plans(id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return db.query(TestPlan).filter(TestPlan.project_id == id).all()


@projects_router.post("/{id}/plans", response_model=TestPlanOut, status_code=status.HTTP_201_CREATED)
def create_plan(id: int, body: TestPlanCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
