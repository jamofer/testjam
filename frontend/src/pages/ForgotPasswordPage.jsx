  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-800">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 shadow-md rounded-xl p-8 w-80 space-y-4">
        ...
        <h1 className="text-base font-semibold text-gray-800 dark:text-gray-100 text-center">{t('forgot.title')}</h1>
        {requestReset.isSuccess ? (
          <p className="text-sm text-gray-700 dark:text-gray-200">{t('forgot.sent')}</p>
        ) : (
          <>
            <p className="text-sm text-gray-600 dark:text-gray-300">{t('forgot.intro')}</p>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1" htmlFor="email">{t('forgot.email')}</label>
              <input
                id="email"
                name="email"
                type="email"
            ...
            >
              {requestReset.isPending ? t('forgot.sending') : t('forgot.send')}
            </button>
          </>
        )}
