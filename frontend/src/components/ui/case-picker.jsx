  ...
  return (
    <div className={depth > 0 ? "border rounded-lg overflow-hidden" : ""}>
      <details className="group/suite" onToggle={() => loadCases(suite.id)}>
        <summary className={`flex items-center gap-2 cursor-pointer select-none list-none font-medium text-gray-800 bg-gray-50 hover:bg-gray-100 transition-colors ${depth === 0 ? "px-3 py-2 text-sm" : "px-3 py-1.5 text-xs"}`}>
        ...
        <div className="px-2 py-1 space-y-0.5">
          {suite.children.length > 0 && (
            <div className="pl-4 mb-1 space-y-1.5 border-l-2 border-gray-100">
          ...
          {allExcluded && (
            <p className="text-xs text-gray-400 py-1 px-2">All cases already added</p>
          )}
          {visible.map(tc => (
            <label key={tc.id} className="flex items-center gap-2 rounded px-2 py-1.5 hover:bg-gray-50 cursor-pointer transition-colors">
              <input
                type="checkbox"
                className="shrink-0"
